...
export const createProject = p => {
 const r = db.prepare(`INSERT INTO projects(name,repository,branch,port,build_command,start_command,project_path) VALUES(?,?,?,?,?,?,?)`).run(p.name,p.repository,p.branch||"main",p.port,p.buildCommand||"",p.startCommand||"npm start",p.projectPath);
 return getProject(r.lastInsertRowid);
};
export const updateProject = (id, fields) => {
 const allowed = {status:"status", processId:"process_id", port:"port", buildCommand:"build_command", startCommand:"start_command"};
 const entries = Object.entries(fields).filter(([k,v]) => allowed[k] && v !== undefined);
 if (!entries.length) return getProject(id);
 const sql = entries.map(([k]) => `${allowed[k]}=?`).join(", ");
 db.prepare(`UPDATE projects SET ${sql}, updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(...entries.map(([,v])=>v), id);
 return getProject(id);
};
export const deleteProject = id => db.prepare("DELETE FROM projects WHERE id=?").run(id).changes > 0;
export const nextPort = (start=31000,end=31999) => {
 const used = new Set(listProjects().map(p=>p.port));
 for (let p=start;p<=end;p++) if(!used.has(p)) return p;
 throw new Error("No free project ports");
};
export const setEnv = (projectId,name,value) => db.prepare(`INSERT INTO environment_variables(project_id,variable_name,variable_value) VALUES(?,?,?) ON CONFLICT(project_id,variable_name) DO UPDATE SET variable_value=excluded.variable_value`).run(projectId,name,value);
export const getEnv = projectId => db.prepare("SELECT variable_name, variable_value FROM environment_variables WHERE project_id=?").all(projectId);
